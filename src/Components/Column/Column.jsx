...
const Column = ({ title, tasks, users }) => {
  if (!Array.isArray(tasks)) return null;  // Ensure tasks is an array
  
  return (
    <div className="column">
      <div className="column-header">
        
        <p>{title} <span className='count'>{tasks.length}</span></p>
        <div className='svgs'>
      ...
      <div className="column-body">
            {tasks.map(task => <Card key={task.id} task={task} users={users} />)}
      </div>
    </div>
  );
};
